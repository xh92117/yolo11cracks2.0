...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if nc and nc != self.yaml["nc"]:
            ...
            raise ValueError("'nc' not specified. Define number of classes %s. ex. nc=80 for coco dataset." % nc)
        ...
        if isinstance(m, (Detect, Segment, Pose, OBB)):
            ...
            self.stride = torch.tensor([32])  # default stride for i.e. IPT, RTDETR
        ...
        # Load task specific modules
        if isinstance(m, (Detect, Segment, Pose, OBB)):
            self.init_criterion()  # init detection loss
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            A2C2f_DYT, A2C2f_CGLU_DYT, A2C2f_DFFN_DYT, C2fMSA
        ) + C3K2_CLASS):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
